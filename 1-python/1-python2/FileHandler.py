...
try:
     all_the_text = file_object.read( )   #读取所有内容
finally:
     file_object.close( )
...
file_object = open('abinfile', 'rb')    #读固定字节
try:
    while True:
...
list_of_all_the_lines = file_object.readlines( )    #读每行,如果文件是文本文件，还可以直接遍历文件对象获取每行
...
output = open('data', 'w+') #追加写文件

file_object = open('thefile.txt', 'w')  #写数据
...
例如需要在一台物理内存为2GB 的机器上处理一个 2GB 的日志文件，我们可能希望每次只处理其中 200MB 的内容。
在Python 中，内置的 File 对象直接提供了一个readlines(sizehint) 函数来完成这样的事情。以下面的代码为例：
